<script lang="ts">
  ...
  $: levelProgress = Math.min(100, ((progressData.points / progressData.nextLevelPoints) * 100));
  ...
  const completeMission = (missionId) => {
    ...
    const missionIndex = todaysMissions.findIndex(m => m.id === missionId);
    if (missionIndex !== -1) {
      const mission = {...todaysMissions[missionIndex], completed: true};
      ...
      if (progressData.points >= progressData.nextLevelPoints) {
        ...
        showToast('🎉 Level Up! You reached level ' + progressData.level, 'success');
      }
    }
  };
  ...
  const updateWater = (change) => {
    const newValue = Math.max(0, Math.min(totalWaterGoal, waterGlasses + change));
    ...
    if (newValue !== waterGlasses) {
      ...
      showToast(`Water intake updated: ${waterGlasses}/${totalWaterGoal} glasses`, 'info');
      
      if (waterGlasses === totalWaterGoal && waterMission && !waterMission.completed) {
  ...
  const setMood = (mood) => {
    ...
    if (mood.label === 'Anxious' || mood.label === 'Sad') {
      ...
      if (!todaysMissions.some(m => m.id === 'breathing')) {
        ...
        showToast('Added a breathing exercise to help with your mood', 'info');
      }
    }
  };
  ...
  let todaysMissions = [
    { 
      ...
      color: '#8b5cf6', 
      completed: false,
      description: 'Start your day with a calm mind. Sit comfortably, focus on your breath for 10 minutes.',
      reward: 50,
      category: 'mindfulness'
    },
    { 
      ...
      icon: 'fa-tint', 
      title: 'Stay Hydrated', 
      subtitle: `${waterGlasses}/${totalWaterGoal} glasses`, 
      color: '#34d399', 
      completed: false,
      description: 'Drink 8 glasses of water throughout the day to stay hydrated and energized.',
      reward: 40,
      category: 'physical'
    },
    { 
      ...
      subtitle: 'Record a statement', 
      color: '#ff79c6', 
      completed: false,
      description: 'Speak your daily affirmation aloud to reinforce positive thoughts.',
      reward: 30,
      category: 'mental'
    },
    { 
      ...
      color: '#f97316', 
      completed: false,
      description: 'Have a balanced meal and take a photo to verify completion.',
      reward: 45,
      category: 'physical'
    }
  ];
  ...
  let characters = [
    ...
    { id: 3, name: 'Gratitude Guardian', icon: '🙏', unlocked: false, level: 0 },
    { id: 4, name: 'Sleep Sorcerer', icon: '💤', unlocked: false, level: 0 },
    { id: 5, name: 'Nutrition Ninja', icon: '🥗', unlocked: true, level: 1 },
...
<div class="phone-container">
  ...
  <div class="app-container">
      ...
      <div class="header-right">
        ...
        <SignedIn>
          <div class="user-stats">
            ...
            <div class="stat user-level" title="Level {progressData.level}">
    ...
    <section class="section card level-card">
      ...
      <div class="progress-bar" role="progressbar" aria-valuenow={progressData.points} aria-valuemin="0" aria-valuemax={progressData.nextLevelPoints}>
    ...
    <section class="section card mood-card">
      ...
      <div class="mood-selection">
        {#each moodOptions as mood}
          <button 
            class="mood-option {currentMood === mood.label ? 'selected' : ''}" 
            style:background={currentMood === mood.label ? mood.color : 'transparent'}
    ...
    <section class="section card progress-card">
      <div class="header-row">
        ...
        <span class="counter">{progressData.completed}/{progressData.total}</span>
      </div>
      <div class="progress-bar" role="progressbar" aria-valuenow={progressData.completed} aria-valuemin="0" aria-valuemax={progressData.total}>
      ...
      <div class="markers">
        {#each Array(progressData.total) as _, i}
          <div class="marker {i < progressData.completed ? 'done' : ''}" aria-hidden="true"></div>
        {/each}
      </div>
      <div class="stats-row">
        <div class="stat-row"><i class="fas fa-fire"></i> Streak: {progressData.streak}d</div>
        <div class="stat-row"><i class="fas fa-gem"></i> Points: {progressData.points}</div>
      </div>
    </section>

    <div class="tabs">
      <button class="tab {selectedTab === 'today' ? 'active' : ''}" on:click={() => selectedTab = 'today'}>
        Today's Missions
      </button>
      <button class="tab {selectedTab === 'completed' ? 'active' : ''}" on:click={() => selectedTab = 'completed'}>
        Completed
      </button>
      <button class="tab {selectedTab === 'upcoming' ? 'active' : ''}" on:click={() => selectedTab = 'upcoming'}>
        Upcoming
      </button>
    </div>
        ...
        <div class="missions-grid">
          {#each todaysMissions as mission}
            <div 
              ...
              on:keydown={(e) => e.key === 'Enter' && openMissionDetails(mission)}
              tabindex="0"
              role="button"
    ...
    <section class="section card">
      ...
      <div class="characters-preview">
        {#each characters.filter(c => c.unlocked).slice(0, 3) as character}
          <div class="character">
  ...
  <nav class="nav-bar">
    {#each navItems as nav}
      <div
        class="nav-item {nav.active ? 'active' : ''} {nav.isAdd ? 'add' : ''}"
        on:click={() => handleNavClick(nav.label || 'add')}
        on:keydown={(e) => e.key === 'Enter' && handleNavClick(nav.label || 'add')}
        tabindex="0"
        role={nav.isAdd ? "button" : "link"}
        aria-label={nav.isAdd ? "Add new activity" : `Navigate to ${nav.label}`}
      >
  ...
  {#if toast.show}
    <div class="toast {toast.type}" transition:scale={{ duration: 200, start: 0.8 }}>{toast.message}</div>
  {/if}

  {#if showMissionDetails}
    <div class="modal-overlay" transition:fade={{ duration: 200 }} on:click={() => showMissionDetails = false}>
      <div class="modal-content" on:click|stopPropagation>
        {#if currentMission}
          <div class="modal-header" style:background={currentMission.color}>
            <i class="fas {currentMission.icon} modal-icon"></i>
            <h2>{currentMission.title}</h2>
            <button class="close-button" on:click={() => showMissionDetails = false}>
          ...
          <div class="modal-body">
            ...
            {#if currentMission.id === 'water'}
              <div class="water-tracker">
                ...
                <div class="water-controls">
                  <button class="water-btn" on:click={() => updateWater(-1)}>
                    <i class="fas fa-minus"></i>
                  </button>
                  <div class="water-display">
                    {#each Array(totalWaterGoal) as _, i}
                      <div class="water-glass {i < waterGlasses ? 'filled' : ''}">
            ...
            {#if currentMission.id === 'affirmation'}
              <div class="affirmation-section">
                ...
                <p class="affirmation-text">"{affirmationText}"</p>
                <button 
                  class="record-button {isSpeaking ? 'recording' : ''}" 
                  on:click={startVoiceDetection}
                  disabled={isSpeaking}
                >
                  <i class="fas {isSpeaking ? 'fa-microphone-alt' : 'fa-microphone'}"></i>
                  {isSpeaking ? 'Recording...' : 'Speak Affirmation'}
                </button>
              </div>
            {/if}
            ...
            {#if !currentMission.completed}
              <button class="complete-button" on:click={() => completeMission(currentMission.id)}>
                Mark as Completed
              </button>
            {:else}
  ...
  {#if showRewards}
    <div class="modal-overlay" transition:fade={{ duration: 200 }} on:click={() => showRewards = false}>
      <div class="modal-content rewards-modal" on:click|stopPropagation>
        <div class="modal-header">
          <h2>Character Collection</h2>
          <button class="close-button" on:click={() => showRewards = false}>
            <i class="fas fa-times"></i>
          </button>
        </div>
        <div class="modal-body">
          <div class="characters-grid">
            {#each characters as character}
              <div class="character-card {character.unlocked ? '' : 'locked'}">
                <div class="character-icon">{character.unlocked ? character.icon : '❓'}</div>
                <div class="character-name">{character.unlocked ? character.name : 'Locked'}</div>
                {#if character.unlocked}
                  <div class="character-level">Level {character.level}</div>
                {:else}
                  <div class="unlock-hint">Keep completing missions to unlock</div>
                {/if}
              </div>
            {/each}
          </div>
...
<style>
  :global(*) { margin:0; padding:0; box-sizing:border-box; font-family:'Inter',sans-serif; }
  :root {
  ...
  .phone-container {
    ...
    margin:2rem auto; border-radius:40px;
  ...
  .status-bar {
    ...
    padding:0 16px; background:rgba(255,255,255,0.7);
    color:var(--text-dark);
  ...
  .app-container {
    padding:16px 16px 0; overflow-y:auto;
  ...
  .btn.sign-in:hover { transform: translateY(-2px); }

  .user-stats { display:flex; gap:6px; }
  .user-stats .stat {
    display:flex; align-items:center; gap:4px;
    background:var(--card); padding:3px 6px;
  ...
  .level-card {
    padding:12px 16px;
    background: linear-gradient(120deg, rgba(99, 102, 241, 0.1), rgba(52, 211, 153, 0.1));
    margin-bottom:16px;
  }
  .level-header {
    display:flex; justify-content:space-between; align-items:center;
  ...
  .progress-bar {
    ...
    background:var(--bg-light); border-radius:5px;
  ...
  .marker {
    ...
    background: #fff; border:2px solid var(--bg-light);
    border-radius:50%;
  }
  .marker.done {
  ...
  .stats-row .stat-row i { margin-right:4px; }
  ...
  .mission-item:hover, .mission-item:focus-visible { 
    transform: translateY(-2px); 
    box-shadow:0 4px 16px var(--shadow-sm);
    outline: 2px solid var(--accent);
  }
  .mission-item.completed { border:1px solid #0cff00; }
  ...
  .nav-item.add {
    ...
    background: linear-gradient(135deg, var(--primary-start), var(--primary-end));
    border-radius:50%; transform: translateY(-20%);
    display:flex; justify-content:center; align-items:center;
    color:#fff; box-shadow:0 4px 16px var(--shadow-sm);
  }
